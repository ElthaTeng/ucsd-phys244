...
void write_input(int idx_N, int idx_T, int idx_n){
  ...
  float N_value = idx_N * 0.2 + 16; // Column densities are set to vary from 16 to 20 cm-2 in log space with step = 0.2 dex
  float n_value = idx_n * 0.2 + 2; // Volume densities are set to vary from 2 to 5 cm-3 in log space with step = 0.2 dex
  ...
  myfile.open ("input_coarse/" + str_N + "_" + str_T + "_"+ str_n + ".inp");
  myfile << "co.dat \n"; 
  myfile << "output_coarse/" + str_N + "_" + str_T + "_"+ str_n + ".out \n"; 
  myfile << "100 300 \n";
  myfile << str_T + "\n";
  myfile << "1 \n"; 
  myfile << "H2 \n";
...
void compute(int idx_N, int idx_T, int idx_n){
  ...
  float N_value = idx_N * 0.2 + 16;
  float n_value = idx_n * 0.2 + 2;
  ...
  system( ("radex < input_coarse/" + str_N + "_" + str_T + "_"+ str_n + ".inp").c_str() );
...
float read_flux(int idx_N, int idx_T, int idx_n){
  ...
  file_radex.open("output_coarse/" + str_N + "_" + str_T + "_"+ str_n + ".out");
...
int main ( int argc, char *argv[] )
{
  ...
  // Write execution time for each step into an output log file
  outfile.open ("main_coarse.log");
  ...
  outfile << "Total elapsed time: " << duration_tot.count() << " microsec \n"; 
  outfile.close();
